...
boolean exec_executeBasicCommand(String &com)
{
  ...
  if (com.startsWith(CMD_CHANGELENGTH))
    ...
    exec_setPosition();
  else if (com.startsWith(CMD_SETHOMEPOINT))
    exec_setHomePoint();
#ifdef PENLIFT
...
void exec_setMachineStepsPerRevFromCommand()
{
  ...
  eeprom_loadMachineSpecFromEeprom();
  #ifdef DEBUG
  Serial.print(F("Steps Per Rev set to "));
...
void exec_changePenWidth()
{
  ...
  Serial.print(F("Pen width set to "));
...
void exec_setHomePoint()
{
  ...
  #ifdef DEBUG_COMMS
  Serial.print(F("Home point set to ("));
...
{
  long lenA = atol(inParam1);
  long lenB = atol(inParam2);
